...
def main(url, prefix, output_path, template):
    ...
    for dataset in data_json:
        ...
        if dataset["webService"].endswith(".pdf"):
            print "skipping {d} because it's a PDF!".format(d=dataset['title'])
            continue
        ...
        for dist in distribution_list:
            if dist["format"] == "ZIP":
                ...
                elements["formname"][0].text = "Download"
                ...
                elements["formname"][2].text = "Information"
        ...
        if dataset["description"]:
            abstract_soup = BeautifulSoup(dataset["description"],"lxml")
